...
import { canUnvoidSavedQuickTurnQuote } from "@/lib/quickTurnQuoteCalculator/permissions";
import { unvoidSavedQuickTurnQuote } from "@/lib/repositories/quickTurnQuoteCalculatorRepo";
...
export async function POST(
  ...
  if (!canUnvoidSavedQuickTurnQuote(auth)) {
  ...
  try {
    ...
    await unvoidSavedQuickTurnQuote(id, {
    ...
    return NextResponse.json(
      { error: err?.message || "Failed to unvoid saved Quick Turn quote." },
      { status: 400 }
    );
  }
}
