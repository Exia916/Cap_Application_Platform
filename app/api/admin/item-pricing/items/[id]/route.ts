...
export async function GET(req: NextRequest, ctx: Ctx) {
  ...
  try {
    const row = await getItemPricingItem(await routeId(ctx));
    if (!row) return NextResponse.json({ error: "Item not found." }, { status: 404 });
    return NextResponse.json(row);
  } catch (err: any) {
    return errorJson(err, "Failed to load item.", 500);
  }
}
...
export async function PUT(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const updated = await updateItemPricingItem(await routeId(ctx), withAudit(body, auth!));
    return NextResponse.json(updated);
  } catch (err: any) {
    return errorJson(err, "Failed to update item.");
  }
}
