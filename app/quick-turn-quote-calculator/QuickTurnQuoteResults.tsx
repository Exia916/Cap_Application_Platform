...
function itemLabel(item: QuickTurnCalculatedItem, index: number) {
  return `${index + 1}. ${item.baseItem.itemCode}`;
}
        ...
        <table className="table-clean">
          ...
          <tbody>
            <tr>
              <td>Base Item</td>
              <td>
                <strong>{item.baseItem.itemCode}</strong>
                <div className="text-muted">{item.baseItem.fabricDescription || "No description"}</div>
              </td>
              <td>—</td>
              <td>Base item price</td>
...
function accessoryBreakdownRows(accessory: AccessoryBreakdownSource): Array<{ label: string; amount: number; formula?: string }> {
  ...
  switch (accessory.pricingMethod) {
    ...
    case "FLAT_PER_UNIT":
    default:
      return [{ label: "Unit price", amount: calculated }];
  }
}
      ...
      {result.items.map((item, index) => (
        <section key={item.clientItemId || index} className="record-section-card">
          ...
          <div className="record-badge-row" style={{ marginBottom: 12 }}>
            <RateMeta label="Base" value={fmtMoneyPrecise(item.baseUnitPrice)} />
    ...
    <div className="record-content">
      ...
      {row.items.map((item, index) => (
        <section key={item.id} className="record-section-card">
          <div className="record-section-header">
            <div>
              <h2 className="record-section-title">Quote Item {index + 1}. {item.baseItemCode}</h2>
              ...
              <table className="table-clean">
                ...
                <tbody>
                  <tr>
                    <td>Base Item</td>
                    <td>
                      <strong>{item.baseItemCode}</strong>
                      <div className="text-muted">{item.baseItemDescription || "No description"}</div>
                    </td>
                    <td>—</td>
                    <td>Base item price</td>
...
export function SavedQuoteMeta({ row }: { row: SavedQuickTurnQuoteDetail }) {
  return (
    <div className="record-meta-grid">
      <MetaItem label="Quote #" value={row.quoteNumber} />
      <MetaItem label="Quote Name" value={row.quoteName} />
